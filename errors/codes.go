...
func InvalidArgument(reason, format string, a ...interface{}) error {
	return xerrors.WithStack(&StatusError{
		Code:    3,
		Reason:  reason,
		Msg: fmt.Sprintf(format, a...),
	})
...
func DBHandleFail(reason, format string, a ...interface{}) error {
	return xerrors.WithStack(&StatusError{
		Code:    17,
		Reason:  reason,
		Msg: fmt.Sprintf(format, a...),
...
func IsDBHandleFail(err error) bool {
	if se := new(StatusError); errors.As(err, &se) {
		return se.Code == 17
	}
	return false
